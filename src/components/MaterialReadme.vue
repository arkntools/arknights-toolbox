<template>
  <ul class="mdui-typo mdui-m-y-0" style="font-size:14px">
    <template v-if="$root.localeCN">
      ...
      <li>点击一次材料名字旁边的<b>{{$t('common.synthesize')}}</b>按钮可模拟全部合成（扣除原材料，增加产物）</li>
      <li>点击材料卡片右侧的掉落信息可显示相关关卡的详细掉落情况，以及关卡性价比（越高越好，仅供参考）</li>
      <li><span v-theme-class="$root.color.pinkText">粉色字材料</span>代表该材料存在原始需求，而不仅仅是上位材料所需的合成材料</li>
      <li><b>预设</b>用于计算干员精英化/技能升级所需材料，添加预设将会丢弃当前所有的<b>需求</b>输入，点击已添加的预设可以进行编辑；在<b>重置需求&amp;已有</b>或<b>重置需求</b>后，预设将一并被清空</li>
      <li><b>{{$t('cultivate.panel.button.farmCalculation')}}</b>可以计算最优的刷图路线，结果仅供参考，稀有度筛选决定了计算时是否考虑该级材料的向上合成，掉落概率数据来自<a href="https://penguin-stats.io/" target="_blank">企鹅物流数据统计</a></li>
    </template>
    <template v-else-if="$root.locale === 'ja'">
      ...
      <li>材料カードの右に「ステージにドロップ詳細」と「ステージの価値」を表示し、高ければ高いほどがいい（参考のみ）。</li>
      ...
      <li><b>「{{$t('cultivate.panel.button.farmCalculation')}}」ボタン</b>で最優先のルートをご計算できますが、参考のみ。レアリティのフィルタでは、このレアリティの材料の上向き合成が計算で考慮されるかどうかを決定します。オリジナルデータは<a href="https://penguin-stats.io/" target="_blank">ペンギン急便データ統計処理部門</a>から。</li>
    </template>
    <template v-else>
      ...
      <li><span v-theme-class="$root.color.pinkText">Material name with a pink color</span> means it has original demand, but not only be used to synthesize.</li>
      ...
      <li><b>"{{$t('cultivate.panel.button.farmCalculation')}}"</b> is a function to calculate the optimal missions by linear programming. Rarity filter determines whether the upward synthesis of this rarity of material is considered in the calculation. All data come from <a href="https://penguin-stats.io/" target="_blank">penguin stats</a>.</li>
    </template>
  </ul>
</template>
